...
#define MINIMUM_TASK_ID ((uint32_t)2)
  ...
  private:
    ...
    static void PeriodicTaskWrapper(SingleShotTimer<PRIORITY, THREAD> *const timer, const uint32_t interval, const std::function<bool(void)> task, const uint32_t priority = 0)
    {
    ...
    static void PeriodicTaskWrapperAdv(SingleShotTimer<PRIORITY, THREAD> *const timer, const std::function<std::tuple<bool, uint32_t, uint32_t>(void)> task)
    {
  ...
  public:
    // Start periodic task
    void PeriodicTask(const uint32_t interval, const std::function<bool(void)> task, const uint32_t priority = 0)
    {
        try {
    ...
    // Start periodic task
    void PeriodicTaskAdv(const std::function<std::tuple<bool, uint32_t, uint32_t>(void)> task, const uint32_t priority = 0)
    {
