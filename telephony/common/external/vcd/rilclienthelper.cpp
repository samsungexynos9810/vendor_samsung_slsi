...
int RilClientHelperDLImpl::Send(void* client, unsigned msgId, void* data, size_t length, Rilc_OnResponse handler)
{
    ...
    if (m_handle != NULL && m_fpIRILC_Send != NULL) {
        return m_fpIRILC_Send(client, msgId, data, length, handler, RIL_SOCKET_1);
    }
    return RILC_STATUS_FAIL;
}
...
RilClientHelper *RilClientHelperFactory::CreateHelperInstance()
{
    RilClientHelper *instance = new RilClientHelperDLImpl();
    if (instance != NULL) {
