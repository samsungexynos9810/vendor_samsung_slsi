...
public class SelectSimPreferenceFragment extends PreferenceFragment {
    ...
    public void onResume() {
        mPrefSim1 = findPreference(DataRetryModeConstants.KEY_SIM1);
        mPrefSim2 = findPreference(DataRetryModeConstants.KEY_SIM2);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        DataRetryModeActivity activity = (DataRetryModeActivity) getActivity();
        if (preference.getKey().equals(DataRetryModeConstants.KEY_SIM1)) {
            activity.onNextFragment(mSimOperator1, SubscriptionManager.from(getContext()).getSubId(SIM1)[0]);
        } else if (preference.getKey().equals(DataRetryModeConstants.KEY_SIM2)) {
