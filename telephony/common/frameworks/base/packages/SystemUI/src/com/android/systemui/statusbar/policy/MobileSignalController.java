...
public class MobileSignalController extends SignalController<
    ...
    // of code.
    private int mDataVoiceType = TelephonyManager.NETWORK_TYPE_UNKNOWN;
    ...
    private final static String BUILD_CARRIER = SystemProperties.get("ro.vendor.config.build_carrier");
    ...
    public MobileSignalController(Context context, Config config, boolean hasMobileData,
        ...
        mLastState.voiceIconGroup = mCurrentState.voiceIconGroup = mDefaultIcons;
    ...
    private void mapIconSets() {
        ...
        if (BUILD_CARRIER.equals("att") || BUILD_CARRIER.equals("tmo")) {
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_EVDO_0, TelephonyIcons.THREE_G_TWO);
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_EVDO_A, TelephonyIcons.THREE_G_TWO);
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_EVDO_B, TelephonyIcons.THREE_G_TWO);
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_EHRPD, TelephonyIcons.THREE_G_TWO);
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_UMTS, TelephonyIcons.THREE_G_TWO);
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_TD_SCDMA, TelephonyIcons.THREE_G_TWO);
        } else {
        ...
        if (!mConfig.showAtLeast3G) {
            ...
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_IDEN, TelephonyIcons.G);
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_GSM, TelephonyIcons.G);

            mDefaultIcons = TelephonyIcons.UNKNOWN;
        } else {
            if (BUILD_CARRIER.equals("att") || BUILD_CARRIER.equals("tmo")) {
                mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_UNKNOWN,
                        TelephonyIcons.THREE_G_TWO);
                ...
                mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_GPRS,
                        TelephonyIcons.THREE_G_TWO);
                mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_IDEN,
                        TelephonyIcons.THREE_G_TWO);
                mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_GSM,
                        TelephonyIcons.THREE_G_TWO);
        ...
        if (BUILD_CARRIER.equals("att") || BUILD_CARRIER.equals("tmo")) {
            hGroup = TelephonyIcons.THREE_G_TWO;
        } else {
        ...
        if (mConfig.show4gForLte) {
            if (BUILD_CARRIER.equals("att") || BUILD_CARRIER.equals("tmo")) {
                mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_LTE, TelephonyIcons.FOUR_G_LTE);
            } else {
        ...
        mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_NR, TelephonyIcons.NR_5G);
    ...
    public int getCurrentIconId() {
        if (mCurrentState.iconGroup == TelephonyIcons.CARRIER_NETWORK_CHANGE) {
            ...
            /* AOSB basic: We don't use this,
                           because we already have a UI to indicate the data disabled state.
            ...
            */
            boolean cutOut = mCurrentState.inetCondition == 0;
    ...
    public void notifyListeners(SignalCallback callback) {
        ...
        MobileIconGroup voiceIcons = mCurrentState.voiceIconGroup;
        ...
        // Disable the DATA_DISABLED icon to show the current RAT information.
        //boolean showDataIcon = mCurrentState.dataConnected || dataDisabled;
        boolean showDataIcon = mCurrentState.dataConnected || unknowState;
        ...
        showDataIcon = true;
        if (BUILD_CARRIER.equals("cmcc") || BUILD_CARRIER.equals("chnopen")
                ||BUILD_CARRIER.equals("ctc") || BUILD_CARRIER.equals("cu")) {
            // Disable the DATA_DISABLED icon to show the current RAT information.
            //showDataIcon &= mCurrentState.isDefault || dataDisabled;
            showDataIcon &= mCurrentState.connected;
        } else {
            // Disable the DATA_DISABLED icon to show the current RAT information.
            //showDataIcon &= mCurrentState.isDefault || dataDisabled;
        ...
        callback.setMobileDataIndicators(statusIcon, qsIcon, voiceIcons.mDataType, typeIcon, qsTypeIcon,
                activityIn, activityOut, dataContentDescription, description, icons.mIsWide,
                mSubscriptionInfo.getSubscriptionId(), mCurrentState.roaming, mCurrentState.imsRegistration);
    }
    ...
    private final void updateTelephony() {
        ...
        boolean simActState = (SubscriptionManager.from(mContext).getSubActiveState(subId) == SubscriptionManager.ACTIVE)
                    ? true : false;
        if (DEBUG) {
            Log.d(mTag, "updateTelephonySignalStrength: hasService=" + Utils.isInService(mServiceState) + " hasDataService=" + hasDataService() + " hasVoiceService=" + hasVoiceService()
                    +" ss=" + mSignalStrength + " SimState("+slotId+")="+ simstate + " DataType="+mDataNetType + " VoiceType=" +mDataVoiceType + " simActState=" + simActState + " subId=" + subId);
        }
        mCurrentState.connected = Utils.isInService(mServiceState)
                && mSignalStrength != null && simActState;
        if (mCurrentState.connected) {
            if (!mSignalStrength.isGsm() && mConfig.alwaysShowCdmaRssi) {
                mCurrentState.level = mSignalStrength.getCdmaLevel();
            } else {
                mCurrentState.level = mSignalStrength.getLevel();
                if((mCurrentState.level == 0) && (mSignalStrength.isGsm()) && (mPhone.getPhoneType() != PhoneConstants.PHONE_TYPE_GSM)) {
                    Log.d(mTag, "updateTelephonySignalStrength: display the cdma signal strenght instead of gsm");
                    mCurrentState.level = mSignalStrength.getCdmaLevel();
                }
            }
        }
        ...
        if ((!hasDataService() && simstate == TelephonyManager.SIM_STATE_READY) || (isSimLock(simstate)) || (!simActState)) {
        ...
        if ((!Utils.isInService(mServiceState) && simstate == TelephonyManager.SIM_STATE_READY) || (isSimLock(simstate)) || (!simActState)) {
            mCurrentState.voiceIconGroup = TelephonyIcons.UNKNOWN;
        } else if (nr5GIconGroup != null) {
            mCurrentState.voiceIconGroup = nr5GIconGroup;
        } else if (mNetworkToIconLookup.indexOfKey(mDataVoiceType) >= 0) {
            mCurrentState.voiceIconGroup = mNetworkToIconLookup.get(mDataVoiceType);
        } else {
            mCurrentState.voiceIconGroup = mDefaultIcons;
        }
        ...
        if (isCarrierNetworkChangeActive()) {
            ...
            mCurrentState.voiceIconGroup = TelephonyIcons.CARRIER_NETWORK_CHANGE;
        }
        /*
        // Disable the DATA_DISABLED icon to show the current RAT information.
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.println("  mVoiceNetType=" + mDataVoiceType + ",");
    }
    ...
    class MobilePhoneStateListener extends PhoneStateListener {
        ...
        public void onServiceStateChanged(ServiceState state) {
            if (DEBUG) {
                Log.d(mTag, "onServiceStateChanged voiceState=" + state.getVoiceRegState()
                        + " dataState=" + state.getDataRegState() + " subid:"+mSubId);
            }
            mServiceState = state;
            if (mServiceState != null) {
                updateDataNetType(mServiceState.getDataNetworkType());
            }
            mDataVoiceType = state.getVoiceNetworkType();
            updateTelephony();
        }
        ...
        public void onSrvccStateChanged(int state) {
            if (DEBUG) {
                Log.d(mTag, "onSrvccStateChanged: state=" + state);
            }
        ...
        public void onCallStateChanged(int state, String phoneNum) {
            if (DEBUG) {
                Log.d(mTag, "onCallStateChanged: state=" + state + ", phoneNum=" + phoneNum);
            }

            if (state == TelephonyManager.CALL_STATE_IDLE) {
                mCurrentSrvcc = TelephonyManager.SRVCC_STATE_HANDOVER_NONE;
                updateTelephony();
            }
        }
    };
    ...
    static class MobileState extends SignalController.State {
        ...
        boolean userSetup;
        boolean roaming;
        boolean imsRegistration;
        MobileIconGroup voiceIconGroup;
        ...
        public void copyFrom(State s) {
            ...
            imsRegistration = state.imsRegistration;
            voiceIconGroup = state.voiceIconGroup;
        }
        ...
        protected void toString(StringBuilder builder) {
            ...
            builder.append("imsRegistration=").append(imsRegistration).append(',');
        ...
        public boolean equals(Object o) {
            return super.equals(o)
                    ...
                    && ((MobileState) o).imsRegistration == imsRegistration
                    && ((MobileState) o).voiceIconGroup == voiceIconGroup;
        }
    }
}
