...
public class StatusBar extends SystemUI implements DemoMode,
    ...
    private static final String ACTION_AIRPLANE_MODE_CHANGE = "android.intent.action.AIRPLANE_MODE_CHANGE";
    ...
    protected void makeStatusBarView(@Nullable RegisterStatusBarResult result) {
        ...
        filter.addAction(ACTION_AIRPLANE_MODE_CHANGE);
    ...
    private final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)) {
                ...
                int count = mQSPanel.getRecords().size();
                for (int i = 0; i < count; i++) {
                    if ((mQSPanel.getRecords().get(i).tile) instanceof AirplaneModeTile) {
                        (mQSPanel.getRecords().get(i).tileView).setEnabled(false);
                    }
                }
            }
            else if (ACTION_AIRPLANE_MODE_CHANGE.equals(action)) {
                int count = mQSPanel.getRecords().size();
                mPhoneCnt--;
                if (mPhoneCnt == 0) {
                    for (int i = 0; i < count; i++) {
                        if ((mQSPanel.getRecords().get(i).tile) instanceof AirplaneModeTile) {
                            (mQSPanel.getRecords().get(i).tileView).setEnabled(true);
