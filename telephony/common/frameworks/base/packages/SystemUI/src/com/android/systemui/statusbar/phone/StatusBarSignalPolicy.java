...
public class StatusBarSignalPolicy implements NetworkControllerImpl.SignalCallback,
    ...
    @Override
    public void setMobileDataIndicators(IconState statusIcon, IconState qsIcon, int voiceStatusType, int statusType,
            int qsType, boolean activityIn, boolean activityOut, String typeContentDescription,
            String description, boolean isWide, int subId, boolean roaming, boolean imsReg) {
        Log.d(TAG, "setMobileDataIndicators subId("+subId+"), voiceStatusType("+voiceStatusType+") , statusType("+statusType+"), qsType("+qsType+"), imsReg("+imsReg+")");
        MobileIconState state = getState(subId);
        if (state == null) {
            return;
        }
        ...
        state.strengthId = statusIcon.icon;
        state.typeId = statusType;
        state.voiceTypeId = voiceStatusType;
        state.contentDescription = statusIcon.contentDescription;
        state.typeContentDescription = typeContentDescription;
        state.roaming = roaming;
        state.imsRegistration = imsReg;
        state.activityIn = activityIn && mActivityEnabled;
        state.activityOut = activityOut && mActivityEnabled;

        int iconId = 0;
        if (mNetworkController.getMobileDataConnected(subId)) { // This will embed sim state
    ...
    public static class MobileIconState extends SignalIconState {
        ...
        public int hashCode() {
            ...
            return Objects
                    .hash(super.hashCode(), subId, strengthId, typeId, voiceTypeId, roaming, imsRegistration, needsLeadingPadding,
        ...
        @Override public String toString() {
            return "MobileIconState(subId=" + subId + ", strengthId=" + strengthId + ", roaming="
                    + roaming + ", imsRegistration=" + imsRegistration + ", typeId=" + typeId + ", voiceTypeId=" + voiceTypeId + ", visible=" + visible + ")";
        }
    }
}
