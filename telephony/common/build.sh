...
build_carrier=("chnopen" "cmcc" "europen" "vzw" "tmo" "att" "spr")
...
function build_host_tools()
{
  ...
  echo
  echo "OUT_DIR=host_tools make mkdtimg lpmake -j$CPU_JOB_NUM"
  OUT_DIR=host_tools make mkdtimg lpmake -j$CPU_JOB_NUM
  echo
  check_exit
  END_TIME=`date +%s`
  FUNC=${FUNCNAME[0]}
