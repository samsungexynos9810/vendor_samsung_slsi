...
status_t ExynosCameraPlugInConverter::m_convertBufferInfoFromFrame(ExynosCameraFrameSP_sptr_t frame, Map_t *map)
{
    ...
    for (i = 0; i < m_srcCurBufCnt; i++) {
        ...
        if (!strncmp(m_name, "ConverterVPL", 12))
            m_srcBufV4L2Format[i] = rect.colorFormat;
        else
        ...
        CFLOGV(frame, "[%d] src frame(%d) buf[%d, %d, %x, %p, %d byte fd(%d) [%d, %d, %d, %d, %d, %d] %s",
                i,
                frame->getFrameCount(),
                m_srcBufPlaneCnt[i],
                m_srcBufIndex[i],
                m_srcBufV4L2Format[i],
                m_srcBuf[i][0],
                m_srcBufSize[i][0],
                m_srcBufFd[i][0],
    ...
    for (i = 0; i < m_dstCurBufCnt; i++) {
        ExynosCameraBuffer buffer;
        for (; nodeIndex < MAX_CAPTURE_NODE; nodeIndex++) {
            ret = frame->getDstBuffer(m_pipeId, &buffer, nodeIndex);
            if (ret != NO_ERROR || buffer.index <= -1) {
                CLOGV("[F%d B%d] %d: getDstBuffer(%d) fail, ret(%d)",
                        frame->getFrameCount(), buffer.index, nodeIndex, m_pipeId, ret);
                continue;
            }
            break;
        }
        ...
        /* get rect */
        ExynosRect rect;
        ret = frame->getDstRect(m_pipeId, &rect, nodeIndex);
        if (ret != NO_ERROR) {
        ...
        CFLOGV(frame, "[%d] dst frame(%d) buf[%d, %d, %x, %p, %dbyte fd(%d) [%d, %d, %d, %d, %d, %d], %s",
                i,
                frame->getFrameCount(),
                m_dstBufPlaneCnt[i],
                m_dstBufIndex[i],
                m_dstBufV4L2Format[i],
                m_dstBuf[i][0],
                m_dstBufSize[i][0],
                m_dstBufFd[i][0],
...
status_t ExynosCameraPlugInConverter::m_migrateDstBufferInfo(int32_t fromIndex, int32_t toIndex)
{
    if (fromIndex < 0 || fromIndex >= PLUGIN_MAX_BUF) return BAD_VALUE;
    if (toIndex   < 0 || toIndex   >= PLUGIN_MAX_BUF) return BAD_VALUE;
    if (m_dstBufFd[fromIndex][0] <= 0) return BAD_VALUE;

    // copy from src to target
    m_dstBufPlaneCnt[toIndex]   = m_dstBufPlaneCnt[fromIndex]  ;
    m_dstBufIndex[toIndex]      = m_dstBufIndex[fromIndex]     ;
    m_dstBufWStride[toIndex]    = m_dstBufWStride[fromIndex]   ;
    m_dstBufHStride[toIndex]    = m_dstBufHStride[fromIndex]   ;
    m_dstBufV4L2Format[toIndex] = m_dstBufV4L2Format[fromIndex];
    m_dstBufHALFormat[toIndex]  = m_dstBufHALFormat[fromIndex] ;
    m_dstBufRotation[toIndex]   = m_dstBufRotation[fromIndex]  ;
    m_dstBufFlipH[toIndex]      = m_dstBufFlipH[fromIndex]     ;
    m_dstBufFlipV[toIndex]      = m_dstBufFlipV[fromIndex]     ;

    memcpy(&m_dstBufSize[toIndex], &m_dstBufSize[fromIndex], sizeof(int) * PLUGIN_MAX_PLANE);
    memcpy(&m_dstBufFd[toIndex],   &m_dstBufFd[fromIndex],   sizeof(int) * PLUGIN_MAX_PLANE);
    memcpy(&m_dstBufRect[toIndex], &m_dstBufRect[fromIndex], sizeof(int) * 6);
    memcpy(&m_dstBuf[toIndex],     &m_dstBuf[fromIndex],     sizeof(int) * PLUGIN_MAX_PLANE);

    // make src as invalid
    for (int i = 0; i < m_dstBufPlaneCnt[fromIndex]; i++) {
        m_dstBufFd[fromIndex][i] = -1;
        m_dstBuf[fromIndex][i] = nullptr;
    }
    m_dstBufIndex[fromIndex] = -1;
