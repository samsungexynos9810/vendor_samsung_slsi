...
status_t VDIS::execute(Map_t *map)
{
    ...
    Array_buf_addr_t  mapDstBufAddr[2];
    mapDstBufAddr[0] = (Array_buf_addr_t )(*map)[PLUGIN_DST_BUF_1];   //target;
    mapDstBufAddr[1] = (Array_buf_addr_t )(*map)[PLUGIN_DST_BUF_2];   //target;
    ...
    for (int i = 0; i < mapDstBufCnt; i++) {
        VDIS_LOGD("%s(%d): dst[%d/%d] idx(%d)::(adr:%p, P%d, S%d, FD%d [%d, %d, %d, %d, %d, %d] format :%d",
                __FUNCTION__, __LINE__,
                i,
                mapDstBufCnt,
                mapDstBufIndex[i],
    ...
    memcpy(mapDstBufAddr[0][0], mapSrcBufAddr[0][0], mapDstBufSize[0][0]);//y plane
    memcpy(mapDstBufAddr[0][1], mapSrcBufAddr[0][1], mapDstBufSize[0][1]);//cbcr plane
    memcpy(mapDstBufAddr[0][2], mapSrcBufAddr[0][2], mapDstBufSize[0][2]);//metaplane
    memset(mapDstBufAddr[0][0], 0xff, mapDstBufSize[0][0]/2);
...
status_t VDIS::setup(Map_t *map)
{
    ...
    int previewWidth    =   (Data_int32_t     )(*map)[PLUGIN_ARRAY_RECT_X];
    int previewHeight   =   (Data_int32_t     )(*map)[PLUGIN_ARRAY_RECT_Y];
    int fps             =   (Data_int32_t     )(*map)[PLUGIN_FPS];


    VDIS_LOGD("%s(%d) : input size(%d x %d), target Recording(%d x %d) Preview(%d x %d) fps(%d)", __FUNCTION__, __LINE__, hwWidth, hwHeight, width, height, previewWidth, previewHeight, fps);
...
#ifdef VDIS_PREVIEW
        (*map)[PLUGIN_PLUGIN_CUR_DST_BUF_CNT] = (Map_data_t)2;
#else
