...
{
    Mutex::Autolock lock(m_startlock);

    /*
     * start() should be called only when the plugIn in stop/init state.
     * Otherwise, there will be a memory leak from libvdis.
     */
    if (startFlag == false) {
        startFlag = true;
        vdis->start();
        CLOGD("PLUGIN_PARAMETER_KEY_START");
    } else {
        CLOGD("vdis already started");
    }
...
status_t ExynosCameraPlugInVDIS::m_stop(void)
{
    ...
    /*
     *  stop() should be called only when the plugIn in run state.
     */
    if (startFlag == true) {
        startFlag = false;
        vdis->stop();
        CLOGD("PLUGIN_PARAMETER_KEY_STOP");
    } else {
        CLOGD("vdis already stoped");
    }
