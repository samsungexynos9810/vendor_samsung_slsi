...
#ifndef EXYNOS_CAMERA_PLUGIN_VPL_H
#define EXYNOS_CAMERA_PLUGIN_VPL_H
...
#include "libvpl.h" /* external lib header */

namespace android {

/*
 * Class ExynosCameraPlugInVPL
 *
 * This is adjusted "Tempate method pattern"
 */
class ExynosCameraPlugInVPL : public ExynosCameraPlugIn {
public:
    ExynosCameraPlugInVPL() : ExynosCameraPlugIn() {}
    ExynosCameraPlugInVPL(int cameraId, int pipeId, int mode) : ExynosCameraPlugIn(cameraId, pipeId, mode) {
        strncpy(m_name, "VPLPlugIn", (PLUGIN_NAME_STR_SIZE - 1));
        vpl = NULL;
        frameCnt = 0;
    };
    virtual ~ExynosCameraPlugInVPL() { ALOGD("%s", __FUNCTION__); };
...
private:
    ...
    Map_t lastVplResultMap;
};
}
#endif //EXYNOS_CAMERA_PLUGIN_VPL_H
