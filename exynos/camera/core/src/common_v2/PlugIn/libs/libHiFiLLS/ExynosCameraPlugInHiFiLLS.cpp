...
status_t ExynosCameraPlugInHiFiLLS::m_create(void)
{
    ...
    if (property_get("persist.vendor.hifills.debug", prop, "0") > 0) {
...
void ExynosCameraPlugInHiFiLLS::rowDraw(int sX, int eX, int Y)
{
...
void ExynosCameraPlugInHiFiLLS::colDraw(int sY, int eY, int X)
{
    int w = mBufferData.width;
    char *y = mBufferData.y;
    char *uv = mBufferData.uv;
...
void ExynosCameraPlugInHiFiLLS::drawFaceRects(void)
{
