...
status_t ExynosCameraStream::getSize(uint32_t *width, uint32_t *height)
{
    status_t ret = NO_ERROR;
    ...
    if (m_stream == NULL) {
        ...
        *width   = m_stream->width;
        *height  = m_stream->height;
    }
    return ret;
}

status_t ExynosCameraStream::setPhysCameraID(int id)
{
    m_physCameraID = id;
    return NO_ERROR;
}

int ExynosCameraStream::getPhysCameraID()
{
    return m_physCameraID;
}

bool ExynosCameraStream::isPhysCameraStream()
{
    return (m_physCameraID >= 0) ? true: false;
}
...
int ExynosCameraStreamManager::getPhysCameraID(int streamId)
{
    ExynosCameraStream *stream;
    int physCamId;

    if (getStream(streamId, &stream) != NO_ERROR) {
        ALOGE("ERR(%s[%d]):Failed to get stream for streamId (%d)",
...
void ExynosCameraStreamManager::dumpYuvStreamId()
{
    for (int i = ExynosCameraParameters::YUV_0; i < ExynosCameraParameters::YUV_OUTPUT_PORT_ID_MAX ; i++) {
        ALOGD("DEBUG(%s[%d]):YUV StreamMap[%d] = %d ", __FUNCTION__, __LINE__, i, m_yuvStreamIdMap[i]);
    }
}

status_t ExynosCameraStreamManager::getYuvStreamIdList(int *streamMap)
{
    status_t ret = NO_ERROR;

    for (int i = ExynosCameraParameters::YUV_0; i < ExynosCameraParameters::YUV_OUTPUT_PORT_ID_MAX ; i++) {
        streamMap[i] = m_yuvStreamIdMap[i];
    }
...
status_t ExynosCameraStreamManager::m_delete(int id, StreamInfoMap *list, Mutex *lock)
{
    ...
    bool IsPhysicalCameraId = false;
    int lenPhysicalCameraId = 0;
    ...
    if (iter != list->end()) {
        ...
        item->getStream(&stream);

        if (stream->physical_camera_id != NULL) {
             IsPhysicalCameraId = true;
             lenPhysicalCameraId = strlen(stream->physical_camera_id);
         }
    ...
    if ((IsPhysicalCameraId == false || lenPhysicalCameraId == 0) || id < HAL_STREAM_ID_MAX) {
