...
status_t ExynosCameraPipeFusion::m_run(void)
{
    ...
    } else if (scenario== SCENARIO_DUAL_REAR_PORTRAIT || scenario== SCENARIO_DUAL_FRONT_PORTRAIT) {
...
status_t ExynosCameraPipeFusion::m_createFusion(void)
{
    ...
    if (scenario == SCENARIO_DUAL_REAR_ZOOM) {
        if (m_fusionWrapper != NULL && m_fusionWrapper->flagCreate(m_cameraId) == false ) {
            ...
            m_fusionWrapper->create(m_camIdInfo, calData, calDataSize);
            if (ret != NO_ERROR) {
        ...
        if (m_bokehWrapper != NULL && m_bokehWrapper->flagCreate(m_cameraId) == false) {
            ...
            m_bokehWrapper->create(m_camIdInfo, calData, calDataSize);
            if (ret != NO_ERROR) {
                CLOGE("m_fusionWrapper->create() fail");
                return ret;
            }
...
status_t ExynosCameraPipeFusion::m_manageFusion(ExynosCameraFrameSP_sptr_t newFrame)
{
    ...
    for (int i = OUTPUT_NODE_1; i <= OUTPUT_NODE_2; i++) {
        ...
        case FRAME_TYPE_PREVIEW:
            cameraId = m_camIdInfo->cameraId[MAIN_CAM];
            break;
        ...
        case FRAME_TYPE_REPROCESSING_DUAL_MASTER:
            if (i == OUTPUT_NODE_1)
                ...
                cameraId = m_camIdInfo->cameraId[SUB_CAM];
            break;
        ...
        if (m_parameters->getHwConnectionMode(PIPE_MCSC, PIPE_VRA) == HW_CONNECTION_MODE_M2M) {
            ...
            m_parameters->getHwVraInputSize(&vraInputSizeWidth, &vraInputSizeHeight, &src_shot_ext[i].shot,
...
void ExynosCameraPipeFusion::m_manageFusionDualPortrait(ExynosCameraFrameSP_sptr_t newFrame,
                                                        int i,
                                                        int cameraId,
