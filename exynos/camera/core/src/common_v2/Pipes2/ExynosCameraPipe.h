...
typedef struct ExynosCameraPipeInfo {
    ...
    ExynosCameraPipeInfo& operator =(const ExynosCameraPipeInfo &other)
    {
        ...
        pixelCompInfo = other.pixelCompInfo;
...
#if defined(SUPPORT_VIRTUALFD_REPROCESSING) || defined(SUPPORT_VIRTUALFD_PREVIEW)
    int32_t virtualNodeNum[MAX_NODE];
#endif
...
#if defined(SUPPORT_VIRTUALFD_REPROCESSING) || defined(SUPPORT_VIRTUALFD_PREVIEW)
            virtualNodeNum[i] = other.virtualNodeNum[i];
#endif
...
public:
    ...
    virtual status_t        setParameter(__unused int key, __unused void *data, int scenario) {
        android_printAssert(NULL, LOG_TAG, "ASSERT(%s[%d]):Not supported API. use PlugInPipe, assert!!!!", __FUNCTION__, __LINE__);
        return NO_ERROR;
    };
    virtual status_t        getParameter(__unused int key, __unused void *data, int scenario) {
        android_printAssert(NULL, LOG_TAG, "ASSERT(%s[%d]):Not supported API. use PlugInPipe, assert!!!!", __FUNCTION__, __LINE__);
        return NO_ERROR;
    };
    ...
    virtual void            setStandbyHint(bool);
    ...
    virtual status_t        getNodes(ExynosCameraNode **nodes[MAX_NODE]);
#endif
 ...
 */
    ...
    virtual status_t        flush(void) {return NO_ERROR;}
