...
status_t ExynosCameraPipe::setExtControl(struct v4l2_ext_controls *ctrl, __unused enum NODE_TYPE nodeType)
{
    CLOGV("");
...
status_t ExynosCameraPipe::resetThreadRenew(void)
{
    m_threadRenew = 0;
...
status_t ExynosCameraPipe::setHandler(sp<PipeHandler> handler) {
    status_t ret = NO_ERROR;
    CLOGD("do not support %s()", __FUNCTION__);
    return ret;
}

status_t ExynosCameraPipe::getHandler(PIPE_HANDLER::USAGE usage, sp<PipeHandler> &handler)
{
    CLOGD("do not support %s()", __FUNCTION__);
    return NO_ERROR;
}

status_t ExynosCameraPipe::getHandler(PIPE_HANDLER::SCENARIO scenario, sp<PipeHandler> &handler)
{
    CLOGD("do not support %s()", __FUNCTION__);
    return NO_ERROR;
}

status_t ExynosCameraPipe::setShreadNode(int32_t src, int32_t dst)
{
    CLOGD("do not support %s()", __FUNCTION__);
    return NO_ERROR;
}

status_t ExynosCameraPipe::getNodes(ExynosCameraNode **nodes[MAX_NODE])
{
    CLOGD("do not support %s()", __FUNCTION__);
    return NO_ERROR;
}
...
void ExynosCameraPipe::m_dumpRunningFrameList(void)
{
    ...
    CLOGI("Thread running state(%d)", m_mainThread->isRunning());
    CLOGI("FrameQ size(input:%d)", m_inputFrameQ->getSizeOfProcessQ());
...
void ExynosCameraPipe::setStandbyHint(bool flag)
{
    m_standbyHintFlag = flag;
}
...
status_t ExynosCameraPipe::m_transitState(pipe_state_t state)
{
    ...
    if (m_state == state) {
        CLOGV("Skip state transition. curState %d", m_state);
        return NO_ERROR;
    }
