...
status_t ExynosCameraPipeSync::create(int32_t *sensorIds)
{
    ...
    CLOGI("create() is succeed (%d), syncType:%d, makeFrameType:%d", getPipeId(), m_syncType, m_makeFrameType);
...
status_t ExynosCameraPipeSync::start(void)
{
    ...
    m_masterMultiFrameCapture = false;
    m_slaveMultiFrameCapture = false;
    m_availableSyncCountChange = false;
    m_lastTimeStamp = 0;
    m_lastFrameCount = 0;
...
status_t ExynosCameraPipeSync::m_run(void)
{
    ...
    if (m_reprocessing == true) {
        CLOGD("[F%d D%d T%d] TimeStampBoot(%lld)ms",
                newFrame->getFrameCount(), newFrame->getMetaFrameCount(), newFrame->getFrameType(), curMs);
    }
    ...
    case FRAME_STATE_SKIPPED:
        ...
        m_setFrameState(newFrame, FRAME_STATE_SKIPPED);
    ...
    case FRAME_TYPE_INTERNAL_SLAVE:
        switch (newFrame->getDualOperationMode()) {
        case DUAL_OPERATION_MODE_SYNC:
            if (m_reprocessing == true) {
                if ((newFrame->getFrameType() == FRAME_TYPE_INTERNAL)
                        && (newFrame->getMaxFrameIndex() > 1)) {
                    m_masterMultiFrameCapture = true;
                } else if ((newFrame->getFrameType() == FRAME_TYPE_INTERNAL_SLAVE)
                        && (newFrame->getMaxFrameIndex() > 1)) {
                    m_slaveMultiFrameCapture = true;
                }
            }
            ...
            break;
        default:
            /* I can't save internal frame cause of no service buffer */
            m_cleanFrameBySyncType(m_slaveFrameQ, newFrame, m_syncType, false);
            m_cleanFrameBySyncType(m_masterFrameQ, newFrame, m_syncType, false);
            m_pushFrameToOutputQ(newFrame);
            break;
    ...
    if (newFrame->getFrameType() == FRAME_TYPE_REPROCESSING_DUAL_MASTER
        && (newFrame->getMaxFrameIndex() > 1)) {
        if (m_slaveMultiFrameCapture == true || m_availableSyncCountChange == true) {
            m_masterMultiFrameCapture = false;
            m_availableSyncCountChange = true;
        }
    } else if ((newFrame->getFrameType() == FRAME_TYPE_REPROCESSING_DUAL_SLAVE)
        && (newFrame->getMaxFrameIndex() > 1)) {
        if (m_masterMultiFrameCapture == true || m_availableSyncCountChange == true) {
            m_slaveMultiFrameCapture = false;
            m_availableSyncCountChange = true;
        }
    }
...
p_exit:
    ...
    int syncWaitingCount = 0;
    if (m_masterMultiFrameCapture == true || m_slaveMultiFrameCapture == true) {
        syncWaitingCount = newFrame->getMaxFrameIndex();
    } else {
        syncWaitingCount = SYNC_WAITING_COUNT;
        m_availableSyncCountChange = false;
    }

    m_cleanFrameByMinCount(m_slaveFrameQ, syncWaitingCount);
    m_cleanFrameByMinCount(m_masterFrameQ, syncWaitingCount);

    CLOGV("[F%d T%d] syncWaitingCount = %d, masterMulti(%d), slaveMulti(%d))",
            newFrame->getFrameCount(), newFrame->getFrameType(),
            syncWaitingCount, m_masterMultiFrameCapture, m_slaveMultiFrameCapture);
...
void ExynosCameraPipeSync::m_init(MAKE_FRAME_TYPE makeFrameType)
{
    m_lastTimeStamp = 0;
    m_lastFrameCount = 0;
    m_syncTimestampMinCalbTime = SYNC_TIMESTAMP_CALB_TIME_DEFAULT;
...
void ExynosCameraPipeSync::m_pushFrameToOutputQ(ExynosCameraFrameSP_sptr_t frame)
{
    ...
    case FRAME_STATE_SKIPPED:
    case FRAME_STATE_INVALID:
        CLOGV("drop this frame (Cam:%d, Fcount:%d, MetaFrameCount:%d, Type:%d, Time:%lld, State:%d) last(%d, %lld, %d)",
                frame->getCameraId(),
                frame->getFrameCount(),
                frame->getMetaFrameCount(),
                frame->getFrameType(),
    ...
    default:
        if (!(frame->getFrameType() == FRAME_TYPE_PREVIEW_DUAL_SLAVE
            ...
            if (m_lastTimeStamp > currentTimeStamp) {
                CLOGE("detected reversed frame!! (Cam:%d, Fcount:%d, MetaFrameCount:%d, Type:%d, Time:%lld, State:%d) last(%d, %lld, %d)",
                        frame->getCameraId(),
                        frame->getFrameCount(),
                        frame->getMetaFrameCount(),
                        frame->getFrameType(),
                        ...
                        m_lastTimeStamp / 1000000LL,
                        m_lastFrameType);
                m_setFrameState(frame, FRAME_STATE_SKIPPED);
            } else {
    ...
    for (int i = 0; i < numOfProcessQ; i++) {
        ...
        if (flagClean) {
            ...
            if (flagSaveFrame) {
                ...
                if (m_makeFrameType == MAKE_FRAME_PAIR) {
                    ret = tempFrame->setSrcBufferState(getPipeId(), ENTITY_BUFFER_STATE_ERROR);
                    if (ret != NO_ERROR) {
                        CFLOGE(tempFrame, "fail!, ret(%d)", ret);
                    }
                } else {
                    m_setFrameState(tempFrame, FRAME_STATE_SKIPPED);
                }
            }
#if 0
                                           ...
                                           ExynosCameraFrameSP_dptr_t finalFrame)
{
    ...
    for (raw_frame_queue_t::iterator r = rawPairFrameQ->begin(); r != rawPairFrameQ->end(); ++r) {
        ...
        case SYNC_TYPE_TIMESTAMP:
            ...
            if (diffTimeMs <= syncCalbTime)
                iamFound = true;
#if 0
            CLOGD("iamFound %d, compare diff(%lldms) calbTime(%lldms) new [Fcount:%d, MetaFrameCount:%d, Type:%d, Time:%lld]"
    ...
    if (iamFound) {
        ...
        switch (m_makeFrameType) {
        case MAKE_FRAME_ONE_MASTER:
            if (iamMasterFrame) {
            ...
            break;
        case MAKE_FRAME_PAIR:
            if (iamMasterFrame) {
                m_makePairFrame(newFrame, pairFrame);
                ...
                m_makePairFrame(pairFrame, newFrame);
            ...
            break;
        default:
            CLOGD("[newF%d][pairF%d] invalid make frame type %d",
                    newFrame->getFrameCount(), pairFrame->getFrameCount(),
                    m_makeFrameType);
            return BAD_VALUE;
        }
    } else {
...
void ExynosCameraPipeSync::m_makeOneMasterFrame(ExynosCameraFrameSP_sptr_t masterFrame, ExynosCameraFrameSP_sptr_t slaveFrame)
{
    ...
    if (buffer.index < 0) {
        ...
        m_setFrameState(masterFrame, FRAME_STATE_SKIPPED);
        m_setFrameState(slaveFrame, FRAME_STATE_SKIPPED);
    } else {
        ...
        if (ret != NO_ERROR) {
            ...
            masterFrame->setCameraId(slaveFrame->getCameraId(), OUTPUT_NODE_2);
            ...
            masterFrame->copySizeInfo(slaveFrame);
        }
    }
...
void ExynosCameraPipeSync::m_makePairFrame(ExynosCameraFrameSP_sptr_t masterFrame,
                                        ExynosCameraFrameSP_sptr_t slaveFrame)
{
    masterFrame->setPairFrame(slaveFrame);
    slaveFrame->setPairFrame(masterFrame);
}

void ExynosCameraPipeSync::m_setFrameState(ExynosCameraFrameSP_sptr_t frame, frame_status_t state)
{
    if (m_pipeId == PIPE_BAYER_SYNC) return;
