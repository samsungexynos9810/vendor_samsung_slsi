...
public:
    ...
    enum MAKE_FRAME_TYPE {
        MAKE_FRAME_ONE_MASTER,  /* Create one master frame. */
        MAKE_FRAME_PAIR,        /* Set Master / Slave frame as pair frame. */
    };

    ExynosCameraPipeSync()
    {
        m_init(MAKE_FRAME_ONE_MASTER);
    }
    ...
    ExynosCameraPipeSync(
        ...
        MAKE_FRAME_TYPE makeFrameType = MAKE_FRAME_ONE_MASTER) : ExynosCameraSWPipe(cameraId, configurations, obj_param, isReprocessing, nodeNums, camIdInfo)
    {
        m_init(makeFrameType);
    }
...
private:
    void                    m_init(MAKE_FRAME_TYPE makeFrameType);
    ...
    void                    m_makePairFrame(ExynosCameraFrameSP_sptr_t masterFrame, ExynosCameraFrameSP_sptr_t slaveFrame);
    void                    m_setFrameState(ExynosCameraFrameSP_sptr_t frame, frame_status_t status);

private:
    frame_queue_t           *m_masterFrameQ;
    frame_queue_t           *m_slaveFrameQ;
    bool                    m_masterMultiFrameCapture;
    bool                    m_slaveMultiFrameCapture;
    uint64_t                m_lastTimeStamp;
    uint32_t                m_lastFrameCount;
    sync_type_t             m_syncType;
    MAKE_FRAME_TYPE         m_makeFrameType;
    uint32_t                m_lastFrameType;
    bool                    m_availableSyncCountChange;
