...
int gLoggerCount;
ExynosCameraTimeLogger gLogger;
...
ExynosCameraTimeLogger::ExynosCameraTimeLogger()
{
    ...
    m_typeStr[LOGGER_TYPE_SUB_CUMULATIVE_CNT]                           = MAKE_STRING(SUB_CUMULATIVE_CNT);
    ...
    m_categoryStr[LOGGER_CATEGORY_DUAL_FACTORY_START_THREAD_START]      = MAKE_STRING(FACTORY_DUAL_START_THREAD_START);
    ...
    m_categoryStr[LOGGER_CATEGORY_FACTORY_START_THREAD_END]             = MAKE_STRING(FACTORY_START_THREAD_END);
    m_categoryStr[LOGGER_CATEGORY_DUAL_FACTORY_START_THREAD_END]        = MAKE_STRING(DUAL_FACTORY_START_THREAD_END);
    m_categoryStr[LOGGER_CATEGORY_PIPE_CREATE]                          = MAKE_STRING(PIPE_CREATE);
    m_categoryStr[LOGGER_CATEGORY_PIPE_SETUP]                           = MAKE_STRING(PIPE_SETUP);
    m_categoryStr[LOGGER_CATEGORY_PIPE_START]                           = MAKE_STRING(PIPE_START);
    m_categoryStr[LOGGER_CATEGORY_RESULT_CALLBACK]                      = MAKE_STRING(RESULT_CALLBACK);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_OPEN]                           = MAKE_STRING(VIDEO_OPEN);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_EOS]                            = MAKE_STRING(VIDEO_EOS);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_S_INPUT]                        = MAKE_STRING(VIDEO_S_INPUT);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_S_FMT]                          = MAKE_STRING(VIDEO_S_FMT);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_REQBUF]                         = MAKE_STRING(VIDEO_REQBUF);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_STREAMON]                       = MAKE_STRING(VIDEO_STREAMON);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_S_CTRL]                         = MAKE_STRING(VIDEO_S_CTRL);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_SENSOR_STREAMON]                = MAKE_STRING(VIDEO_SENSOR_STREAMON);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_STREAMOFF]                      = MAKE_STRING(VIDEO_STREAMOFF);
    m_categoryStr[LOGGER_CATEGORY_VIDEO_CLOSE]                          = MAKE_STRING(VIDEO_CLOSE);
    m_categoryStr[LOGGER_CATEGORY_FIRST_PREVIEW]                        = MAKE_STRING(FIRST_PREVIEW);
    ...
    for (int i = 0; i < CAMERA_ID_MAX; i++) {
        ...
        m_bufferIndex[i] = 0;
        m_stopFlag[i] = true;
        for (int j = 0; j < LOGGER_TYPE_MAX; j++)
            m_firstCheckFlag[i][j] = true;
    }
...
status_t ExynosCameraTimeLogger::init(int cameraId)
{
    ...
    for (int i = 0; i < LOGGER_TYPE_MAX; i++) {
        m_firstCheckFlag[cameraId][i] = false;
    }
    m_count[cameraId] = 0;
...
status_t ExynosCameraTimeLogger::update(int cameraId, uint64_t key, uint32_t pipeId, LOGGER_TYPE type, LOGGER_CATEGORY category, uint64_t userData)
{
    ...
    case LOGGER_TYPE_INTERVAL:
        ...
        if (m_firstCheckFlag[cameraId][type] == false) {
            timer->start();
            m_firstCheckFlag[cameraId][type] = true;
        } else {
            goto p_logger;
        }
        break;
    ...
    case LOGGER_TYPE_SUB_CUMULATIVE_CNT:
        if (m_firstCheckFlag[cameraId][type] == false) {
            m_count[cameraId] = 1;
            m_firstCheckFlag[cameraId][type] = true;
        } else {
...
p_logger:
    ...
    buffer->timeStamp = systemTime(SYSTEM_TIME_REALTIME) / 1000LL;
    buffer->tid = gettid();
    ...
    if (!bufferIndex) m_firstTimestamp = buffer->timeStamp;
    else              m_lastTimestamp = buffer->timeStamp;

    CLOGV3(cameraId, "TimeStamp:%jd,tid:%d,Key:%jd,Pipe:%d,Type:%s,Cate:%s,CalT:%d",
            buffer->timeStamp,
            buffer->tid,
            buffer->key,
            buffer->pipeId,
...
status_t ExynosCameraTimeLogger::save(int cameraId)
{
    ...
    /* save the file */
    fprintf(fd, "timestamp(ms),threadId,key,pipeId,type,category,calTime(us)\n");
    for (int i = 0; i < TIME_LOGGER_SIZE; i++) {
        buffer = &m_buffer[cameraId][i];
        if (buffer->timeStamp == 0)
            continue;

        fprintf(fd, "%jd,%d,%jd,%d,%s,%s,%d\n",
                buffer->timeStamp,
                buffer->tid,
                buffer->key,
                buffer->pipeId,
