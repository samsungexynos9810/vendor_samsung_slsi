...
ExynosCameraActivitySpecialCapture::ExynosCameraActivitySpecialCapture(int cameraId,
                                                                       ...
                                                                       : ExynosCameraActivityBase(cameraId, camIdInfo, staticInfo)
{
    ...
    m_captureIntent = AA_CAPTURE_INTENT_STILL_CAPTURE;
    m_captureCount = 0;
    m_currentCaptureCount = 0;
    ...
    if (shot_ext != NULL && m_specialCaptureMode == SCAPTURE_MODE_HDR) {
        ...
        case SCAPTURE_STEP_START:
            if (m_multiCaptureMode == false) {
                ...
                if (m_currentCaptureCount < captureCount) {
                    shot_ext->shot.ctl.aa.captureIntent       = this->getCaptureIntent();
                    shot_ext->shot.ctl.aa.vendor_captureCount = captureCount;

                    m_currentCaptureCount++;
                }
        ...
        default:
            m_specialCaptureStep = SCAPTURE_STEP_OFF;
            // if SCAPTURE_MODE_OIS -then-> SCAPTURE_STEP_START, the current mode can be disappear.
            // so, comment out.
            //m_specialCaptureMode = SCAPTURE_MODE_NONE;
            break;
        }
    ...
    switch (m_specialCaptureMode) {
        ...
        case SCAPTURE_MODE_OIS:
            if (m_OISCaptureFcount == 0
                ...
                /*
                HACK : To support LCD LLS Flash capture.
                */
                if (getCaptureIntent() == AA_CAPTURE_INTENT_STILL_CAPTURE_LLS_FLASH
                    && m_cameraId == CAMERA_ID_FRONT) {
                    OISCaptureDelay += LLS_LCD_FLASH_DELAY;
                }
#endif
...
{
    ////////////////////////////////////////////////
    // if SCAPTURE_MODE_OIS mode changed, reset Fcount
    if ((m_specialCaptureMode != SCAPTURE_MODE_OIS && sCaptureModeVal == SCAPTURE_MODE_OIS)||
        (m_specialCaptureMode == SCAPTURE_MODE_OIS && sCaptureModeVal != SCAPTURE_MODE_OIS)) {
        this->resetOISCaptureFcount();
...
ExynosCameraActivitySpecialCapture::SCAPTURE_MODE ExynosCameraActivitySpecialCapture::getCaptureMode(void)
{
    return m_specialCaptureMode;
}
...
{
    CLOGD("waitSignalTime : %llu", (unsigned long long)m_waitSignalTime);
...
void ExynosCameraActivitySpecialCapture::setCaptureIntent(enum aa_capture_intent captureIntent)
{
    CLOGD("m_captureIntent : %d -> %d", m_captureIntent, captureIntent);

    m_captureIntent = captureIntent;
}

enum aa_capture_intent ExynosCameraActivitySpecialCapture::getCaptureIntent(void)
{
    return m_captureIntent;
}

void ExynosCameraActivitySpecialCapture::setCaptureCount(int captureCount)
{
    CLOGD("m_captureCount : %d -> %d", m_captureCount, captureCount);

    m_captureCount = captureCount;
}

int  ExynosCameraActivitySpecialCapture::getCaptureCount(void)
{
