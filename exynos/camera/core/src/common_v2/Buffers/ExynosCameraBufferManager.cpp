...
status_t ExynosCameraBufferManager::setInfo(buffer_manager_configuration_t info)
{
    ...
    int metaPlaneIndex  = 0;
    int debugPlaneIndex = 0;
    int index           = 0;
...
status_t ExynosCameraBufferManager::m_defaultAlloc(int bIndex, int eIndex, unsigned int extraPlane)
{
    ...
    for (bufIndex = bIndex; bufIndex < eIndex; bufIndex++) {
...
#ifdef USE_GIANT_MSCL
        if (isMetaPlane == false && isDebugPlane == false && m_buffer[bufIndex].size[0] >= GIANT_MIN_SIZE) {
            additionalSize = GIANT_MARIN_SIZE;
        }
#endif
        for (int planeIndex = planeIndexStart; planeIndex < planeIndexEnd; planeIndex++) {
            ...
            m_buffer[bufIndex].size[planeIndex] += additionalSize;
        ...
        if (isImagePlane == true) {
            ...
            CLOGD("duration time(%5d msec):(type=%d, bufIndex=%d, size=%.2f, batchSize=%d [0][%d,%d], [1][%d,%d])",
                 (int)durationTime, m_buffer[bufIndex].type, bufIndex,
                 (float)bufferSize / (float)(1024 * 1024), m_buffer[bufIndex].batchSize,
                 m_buffer[bufIndex].fd[0], m_buffer[bufIndex].size[0],
                 m_buffer[bufIndex].fd[1], m_buffer[bufIndex].size[1]);
    ...
    if (m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_RESERVED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_RESERVED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_SYNC_FORCE_RESERVED_TYPE) {
        CLOGD("Allocated Reserved Buffer Size(%ju)", m_sReservedBufferSize);
    } else if (m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_NONCACHED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_SYNC_FORCE_TYPE) {
        CLOGD("Allocated Ion Buffer Size(%ju)", m_sIonBufferSize);
    }
...
status_t ExynosCameraBufferManager::m_defaultFree(int bIndex, int eIndex, unsigned int extraPlane)
{
    ...
    for (bufIndex = bIndex; bufIndex < eIndex; bufIndex++) {
        if (isAvaliable(bufIndex) == false) {
            CLOGE("buffer [bufIndex=%d] in InProcess state(fd%d, addr%p)", bufIndex,
                    m_buffer[bufIndex].fd[0],
                    m_buffer[bufIndex].addr[0]);
    ...
    if (m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_RESERVED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_RESERVED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_SYNC_FORCE_RESERVED_TYPE) {
        CLOGD("Remained Reserved Buffer Size(%ju)", m_sReservedBufferSize);
    } else if (m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_NONCACHED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_TYPE
        || m_buffer[bufIndex - 1].type == EXYNOS_CAMERA_BUFFER_ION_CACHED_SYNC_FORCE_TYPE) {
        CLOGD("Remained Ion Buffer Size(%ju)", m_sIonBufferSize);
    }
func_exit:
...
status_t InternalExynosCameraBufferManager::decreaseOnDemandBuffers(void)
{
    /* Only Additinal buffers on OnDemand will be release in this function */
    /* clear buffers except releasing the memory */
    status_t ret = NO_ERROR;
    ...
    ret = m_decrease(m_indexOffset, m_allocatedBufCount + m_indexOffset);
    if (ret != NO_ERROR) {
        CLOGE("fail to decrease");
    }
...
status_t InternalExynosCameraBufferManager::m_decrease(int sIndex, int eIndex)
{
    CLOGD("IN..");
    ExynosCameraAutoTimer autoTimer(__FUNCTION__);
    ...
    if ((sIndex < m_indexOffset) || (eIndex > m_allocatedBufCount)) {
        CLOGE("Invalid range(%d ~ %d), req:%d, indexOff:%d, alloc:%d",
                sIndex, eIndex, m_reqBufCount, m_indexOffset, m_allocatedBufCount);
        ret = BAD_VALUE;
        goto func_exit;
    }

    // try to find last available index
    while (index < eIndex) {
        if (isAvaliable(index) == false) {
            sIndex = index + 1;
        }
        index++;
    }
    ...
    CLOGI("Buffer %d - %d Will be released, m_reqBufCount%d, m_indexOffset%d, m_allocatedBufCount%d",
            sIndex, eIndex, m_reqBufCount, m_indexOffset, m_allocatedBufCount);

    // release buffer one by one
    while (index < eIndex &&
            m_allocatedBufCount > m_reqBufCount) {
        if (m_free(index, index + 1) != NO_ERROR) {
            CLOGE("m_free failed");
            ret = INVALID_OPERATION;
            goto func_exit;
        }
        ...
        if (m_hasMetaPlane == true) {
            if (m_defaultFree(index, index + 1, m_hasMetaPlane) != NO_ERROR) {
                CLOGE("m_defaultFree failed");
                ret = INVALID_OPERATION;
                goto func_exit;
            }
    ...
    for (auto i : m_availableBufferIndexQ)
        CLOGI("Remained Available Buffer Index : %d", i);
    m_availableBufferIndexQLock.unlock();

    CLOGD("Decrease the buffer succeeded (reqBufCount=%d, m_allocatedBufCount=%d)",
            m_reqBufCount, m_allocatedBufCount);
...
status_t ServiceExynosCameraBufferManager::decreaseOnDemandBuffers(void)
{
    CLOGI("Not supported this function");
    return NO_ERROR;
}
...
status_t SWExynosCameraBufferManager::decreaseOnDemandBuffers(void)
{
    /* Only Additinal buffers on OnDemand will be release in this function */
    /* clear buffers except releasing the memory */
    status_t ret = NO_ERROR;
...
status_t SWExynosCameraBufferManager::setInfo(buffer_manager_configuration_t info)
{
    ...
    for (int bufIndex = m_indexOffset; bufIndex < info.allowedMaxBufCount + m_indexOffset; bufIndex++) {
        ...
        m_swBuffer[bufIndex].bufMgrNm   = m_name; /* for debug */
        m_swBuffer[bufIndex].hasMetaPlane = m_hasMetaPlane;
    }
...
status_t SWExynosCameraBufferManager::m_defaultAlloc(int bIndex, int eIndex, unsigned int extraPlane)
{
    EXYNOS_CAMERA_BUFFER_IN();
    ...
    for (bufIndex = bIndex; bufIndex < eIndex; bufIndex++) {
        ...
        int additionalSize = 0;
#ifdef USE_GIANT_MSCL
        if (isMetaPlane == false && isDebugPlane == false && m_swBuffer[bufIndex].size[0] >= GIANT_MIN_SIZE) {
            additionalSize = GIANT_MARIN_SIZE;
        }
#endif
        for (int planeIndex = planeIndexStart; planeIndex < planeIndexEnd; planeIndex++) {
            ...
            m_swBuffer[bufIndex].size[planeIndex] += additionalSize;
...
status_t SWExynosCameraBufferManager::m_defaultFree(int bIndex, int eIndex, unsigned int extraPlane)
{
    EXYNOS_CAMERA_BUFFER_IN();
...
bool SWExynosCameraBufferManager::m_checkInfoForAlloc(void)
{
    ...
    for (int bufIndex = m_indexOffset; bufIndex < m_reqBufCount + m_indexOffset; bufIndex++) {
        ...
        for (int planeIndex = 0; planeIndex < m_swBuffer[bufIndex].planeCount; planeIndex++) {
            int curPlaneIndex = planeIndex;
            int imagePlaneCount = m_getImagePlaneCount( m_swBuffer[bufIndex].planeCount);
            //Convert to metaPlaneIndex.
            if (m_hasMetaPlane == true && planeIndex == imagePlaneCount)
                curPlaneIndex =m_swBuffer[bufIndex].getMetaPlaneIndex();
            //Convert to debugInfoPlaneIndex.
            if (m_hasDebugInfoPlane == true ){
                if (m_hasMetaPlane == true && planeIndex == (imagePlaneCount+1))
                    curPlaneIndex = m_swBuffer[bufIndex].getDebugInfoPlaneIndex();
                else if (m_hasMetaPlane == false && planeIndex == imagePlaneCount )
                    curPlaneIndex = m_swBuffer[bufIndex].getDebugInfoPlaneIndex();
            }
...
void SWExynosCameraBufferManager::dumpBufferInfo(int fd)
{
    CLOGW("m_allocatedBufCount(%d), m_reservedMemoryCount(%d), m_reqBufCount(%d), " \
            "m_flagNeedMmap(%d), m_hasMetaPlane(%d), m_hasDebugInfoPlane(%d),m_indexOffset(%d)",
            m_allocatedBufCount, m_reservedMemoryCount, m_reqBufCount, m_flagNeedMmap, m_hasMetaPlane, m_hasDebugInfoPlane,m_indexOffset);
    ...
    for (int bufIndex = m_indexOffset; bufIndex < m_allocatedBufCount + m_indexOffset; bufIndex++) {
        int totalPlaneCount = m_getTotalPlaneCount(m_swBuffer[bufIndex].planeCount,
                                                    m_swBuffer[bufIndex].batchSize, m_hasMetaPlane,m_hasDebugInfoPlane);
        for (int planeIndex = 0; planeIndex < totalPlaneCount; planeIndex++) {
...
status_t SWExynosCameraBufferManager::m_increase(int increaseCount)
{
    ...
    if (m_hasDebugInfoPlane == true) {
        if (m_defaultAlloc(m_allocatedBufCount + m_indexOffset,
                            m_allocatedBufCount + m_indexOffset + increaseCount,
                            1 << DEBUG_INFO_PLANE_SHIFT) != NO_ERROR) {
            CLOGE("m_defaultAlloc failed");
            ret = INVALID_OPERATION;
            goto func_exit;
        }
...
status_t SWExynosCameraBufferManager::m_decrease(int sIndex, int eIndex)
{
    CLOGD("IN..");
    ExynosCameraAutoTimer autoTimer(__FUNCTION__);
