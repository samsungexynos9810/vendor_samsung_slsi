...
{
    m_autofocusMgr = new ExynosCameraActivityAutofocus(cameraId, camIdInfo, staticInfo);
    m_flashMgr = new ExynosCameraActivityFlash(cameraId, camIdInfo, staticInfo);
    m_specialCaptureMgr = new ExynosCameraActivitySpecialCapture(cameraId, camIdInfo, staticInfo);
    m_uctlMgr = new ExynosCameraActivityUCTL(cameraId, camIdInfo, staticInfo);
...
bool ExynosCameraActivityControl::getOISCaptureMode(void)
{
    bool ret = false;

    if (m_specialCaptureMgr->getCaptureMode() == ExynosCameraActivitySpecialCapture::SCAPTURE_MODE_OIS) {
        ret = true;
    }
