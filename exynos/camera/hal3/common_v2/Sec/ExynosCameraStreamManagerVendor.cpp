...
int ExynosCameraStreamManager::getOutputPortIdForDualScenario(void)
{
    int outputPortId = -1;
    int startIndex = ExynosCameraParameters::YUV_0;
    int endIndex = ExynosCameraParameters::YUV_MAX;
    int streamId = 0;

    //Priority-1: Try to use Preview Port (if available) as onePortId for DualScenario
    for (int i = startIndex; i < endIndex; i++) {
        if ((m_yuvStreamIdMap[i] % HAL_STREAM_ID_MAX) == HAL_STREAM_ID_PREVIEW) {
            outputPortId = i;
            return  outputPortId;
        }
    }

    //Priority-2: Try to use the YUV Ports (if available) corresponding to logical streams
    for (streamId = 0; streamId < HAL_STREAM_ID_MAX; streamId++) {
        switch (streamId % HAL_STREAM_ID_MAX) {
        case HAL_STREAM_ID_PREVIEW:
        case HAL_STREAM_ID_VIDEO:
        case HAL_STREAM_ID_CALLBACK:
    ...
    //Priority-3: Try to use the YUV Ports (if available) corresponding to physical streams
    for (streamId = 0; streamId < HAL_STREAM_ID_MAX; streamId++) {
        switch (streamId % HAL_STREAM_ID_MAX) {
        case HAL_STREAM_ID_CALLBACK_PHYSICAL:
            for (int i = startIndex; i < endIndex; i++) {
