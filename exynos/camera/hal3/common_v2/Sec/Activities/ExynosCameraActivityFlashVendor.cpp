...
int ExynosCameraActivityFlash::t_func3ABeforeHAL3(void *args)
{
    ...
    if (m_flashPreStatus != m_flashStatus) {
        CLOGD("[Flash][F%d]m_flashReq=%d, m_flashStatus=%d, m_flashStep=%d",
            getMetaDmRequestFrameCount(shot_ext), (int)m_flashReq, (int)m_flashStatus, (int)m_flashStep);
    ...
    if (m_flashReq == FLASH_REQ_OFF) {
        ...
        if (m_flashStatus == FLASH_STATUS_OFF || m_flashStatus == FLASH_STATUS_PRE_CHECK) {
            ...
            /*
             * "Main Flash control" should be synced with CAPTURE INTENT(N)!
             * If HAL applied "Main Flash Control" fastly before CAPTURE INTENT(N) frame,
             * service can get unmatched flashed frame(N).
             * This is because driver try to apply "Main Flash Control" to N-2 frame now.
             * --------------------------------------------------------------------------
             *  @Fail Case (Previous)
             *    0) Capture Intent is STILL_CAPTURE
             *    1) HAL's fast applying scheme with flash control from N
             *    2) Driver's sensor control scheme, N-1 control will be applied to N-3.
             *       (refer to driver's define[SENSOR_REQUEST_DELAY])
             *    3) Flashed Frame. This frame is always N+2 frame from 2)
             *          |     N-3        |     N-2     |       N-1      |        N       |
             *---------------------------------------------------------------------------
             *  Service |                |             |                |0)CAPTURE_INTENT|
             *  HAL     |                |             |1)MAIN FLASH CTL|                |
             *  Driver  |2)MAIN FLASH CTL|             |                |                |
             *  DDK     |                |             |  3)*FLASHED    |                |
             *---------------------------------------------------------------------------
             *  @Success Case (Now)
             *    0) Capture Intent is STILL_CAPTURE
             *    1) Main flash control will be applied by perframe from N
             *    2) Driver's sensor control scheme, N-1 control will be applied to N-3.
             *       (refer to driver's define[SENSOR_REQUEST_DELAY])
             *    3) Flashed Frame. This frame is always N+2 frame from 2)
             *          |     N-3      |     N-2        |       N-1      |        N       |
            ...
            if (   (m_flashStep == FLASH_STEP_MAIN_START)
                && (   shot_ext->shot.ctl.aa.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE
                    || shot_ext->shot.ctl.aa.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE_LLS_FLASH)) {
        ...
        if (m_aeState == AE_STATE_INACTIVE) {
            ...
            goto done;
                   ...
                   || (m_cameraId == CAMERA_ID_FRONT && m_aeState == AE_STATE_SEARCHING)
                   ) {
            ...
            if (m_flashStatus == FLASH_STATUS_OFF || m_flashStatus == FLASH_STATUS_PRE_CHECK) {
                ...
                // refer to upper comments! ("Main Flash control")
                if (   (m_flashStep == FLASH_STEP_MAIN_START)
                    && (   shot_ext->shot.ctl.aa.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE
                        || shot_ext->shot.ctl.aa.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE_LLS_FLASH)) {
...
int ExynosCameraActivityFlash::t_func3AAfterHAL3(void *args)
{
    ...
    /* TODO: Do not refer flashReady state in front flash scenario */
    if (m_cameraId == CAMERA_ID_FRONT)
        shot_ext->shot.dm.flash.vendor_flashReady = 0;
    ...
    if (m_flashStep == FLASH_STEP_OFF) {
        if (shot_ext->shot.dm.aa.vendor_aeflashMode == AA_FLASHMODE_ON_ALWAYS) {
            ...
            if (shot_ext->shot.dm.flash.vendor_firingStable == CAPTURE_STATE_FLASH) {
                CLOGD("[Flash][F%d] is flashed frame aeState(%d)", shot_ext->shot.dm.request.frameCount, shot_ext->shot.dm.aa.aeState);
                m_ShotFcount = shot_ext->shot.dm.request.frameCount;
                m_waitingFlashStableCount = 0;
            }
        }
    ...
    if (m_flashStatus == FLASH_STATUS_PRE_CHECK) {
        ...
        if (shot_ext->shot.dm.aa.aePrecaptureTrigger == AA_AE_PRECAPTURE_TRIGGER_START || m_preTriggerStarted == true) {
            m_preTriggerStarted = true;
            shot_ext->shot.dm.aa.aeState = AE_STATE_PRECAPTURE;
        }
        if (m_cameraId == CAMERA_ID_FRONT) {
            m_flashStatus = FLASH_STATUS_PRE_AE_DONE;
            m_timeoutCount = 0;
            m_ShotFcount = 0;
        } else if (shot_ext->shot.dm.flash.vendor_flashReady == 1 ||
        ...
        bool flagBestFrame = false;
        /* TODO: We need to make common squence for non-AF front flash scenario */
        /*       In this case HAL should send proper result when firingStable is setted. */
        if (m_cameraId == CAMERA_ID_FRONT
                && shot_ext->shot.dm.aa.afMode == AA_AFMODE_OFF
            ...
            CLOGD("[Flash][F%d] checking flashed frame aeState(%d)",
                frameCount, shot_ext->shot.dm.aa.aeState);

            if (   shot_ext->shot.dm.aa.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE
                || shot_ext->shot.dm.aa.captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE_LLS_FLASH) {
                CLOGD("[Flash][F%d] is flashed frame aeState(%d) m_ShotFcount(%d -> %d)",
                    frameCount, shot_ext->shot.dm.aa.aeState, m_ShotFcount, frameCount);

                m_ShotFcount = frameCount;
                m_waitingFlashStableCount = 0;
                flagBestFrame = true;
            }
        } else {
        ...
        if (shot_ext->shot.dm.aa.afState == AA_AFSTATE_PASSIVE_FOCUSED
            ...
            if (flagBestFrame) {
                CLOGD("FLASH_STATUS_MAIN_DONE aeState(%d) afState(%d)",
            ...
            if (m_cameraId != CAMERA_ID_FRONT
                && shot_ext->shot.dm.aa.afState == 0) {// Invalid state of AF, by no focus module.
                CLOGD("FLASH_STATUS_PRE_DONE aeState(%d) afState(%d)",
                        shot_ext->shot.dm.aa.aeState, shot_ext->shot.dm.aa.afState);
                m_flashStatus = FLASH_STATUS_PRE_DONE;
                m_timeoutCount = 0;
            }
