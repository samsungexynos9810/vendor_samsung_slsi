...
int main(int argc, char **argv)
{
  ...
  options.setSOCType(SOCType::ST_NEUS);
  ...
  // currently, only max pool is supported
  bool supported = compiler.isSupportedOperation(OpType::MAX_POOL_2D, opOptions);
  NPUC_LOG(INFO) << "MAX_POOL_2D is supported: true == " << supported << std::endl;
